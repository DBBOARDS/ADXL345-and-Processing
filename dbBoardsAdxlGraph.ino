/*-----------------------------------------------------------------------------------------------------------
  ...
  Summary:
    ...
    to be generated into a live graph in dbBoardsAdxlGraph.pde
...
void loop(){                   // The loop program follows the setup program and repeats forever
  ...
  sendAccelToProcessing();     // Send the x, y, and z values to the dbBoardsAdxlGraph.pde to be displayed
  delay(30);                   // Give the Processing program time to parse and graph the data
}

void sendAccelToProcessing(){  // Send the x, y, and z values to the dbBoardsAdxlGraph.pde to be displayed
  Serial.print("x");           // Tell the Proccessing sketch it is about to recieve an x value
  Serial.print(x);             // Send the x value to dbBoardsAdxlGraph.pde
  Serial.print("y");           // Tell the Proccessing sketch it is about to recieve an y value
  Serial.print(y);             // Send the y value to dbBoardsAdxlGraph.pde
  Serial.print("z");           // Tell the Proccessing sketch it is about to recieve an z value
  Serial.print(z);             // Send the z value to dbBoardsAdxlGraph.pde
}
